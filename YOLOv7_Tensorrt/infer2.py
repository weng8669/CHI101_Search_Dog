...
class TRT_engine():
    ...
    def  predict(self,img,threshold):
        ...
        rit = round(infer_time,2)
        print(infer_time)
        print("Inference Time:", round(infer_time,2),"s")
        
        print("-----------------------------")
        print(f'1 Tudo, Done. ({rit}s)')
        print("-----------------------------")
        # 1 Tudo, Done. (0.205s)
        
        INtime = round(infer_time,2)
...
def visualize(img,bbox_array):
    names = ['Bear', 'Bear_tail', 'Ben', 'Black', 'Black_tail', 'Lele', 'Qbi', 'Sabai', 'Sabai_face', 'Sabai_foot', 'Sabai_tail', 'Tudo']
    for temp in bbox_array:
        ...
        img = cv2.putText(img, names[clas]+" "+str(round(score,2)), (xmin,int(ymin)-5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (105, 237, 249), 1)
        print(names[clas]+" 位置")
        dogname = names[clas]
    return img

trt_engine = TRT_engine('yolov7tiny_fp16.engine')
# trt_engine = TRT_engine('yolov7-nms.trt')
# img = cv2.imread("0c9a5102-image__160.jpg")
img = cv2.imread("0bc.png")
results = trt_engine.predict(img,threshold=0.2)
img = visualize(img,results)

print(results)
# print(results[0][0])
print("-----------------------------")
print(dogname,INtime)
print("-----------------------------")
