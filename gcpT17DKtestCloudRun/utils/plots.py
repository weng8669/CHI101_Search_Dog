...
def plot_one_box(x, img, color=None, label=None, line_thickness=3):
    ...
    label = Dog_CH[label.split(' ')[0]] + ' ' + label.split(' ')[1]
    ...
    if label:
        ...
        font_size = t_size[1]
        font = ImageFont.truetype(r"zonyuan.ttc", font_size, encoding="utf-8")
        t_size = font.getsize(label)
        c2 = c1[0] + t_size[0]+10, c1[1] - t_size[1]
        
        cv2.rectangle(img, c1, c2, color, -1, cv2.LINE_AA)  # filled
        img = Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
        draw = ImageDraw.Draw(img)
        draw.text((c1[0], c2[1]), label, (255,255,255), font=font)
        # draw.text((c1[0], c2[1]), Dog_CH[label.split(' ')[0]] + ' ' + label.split(' ')[1], (255,255,255), font=font)
        # print(Dog_CH[label.split(' ')[0]] + ' ' + label.split(' ')[1])
